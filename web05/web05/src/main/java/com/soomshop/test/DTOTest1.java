...
public class DTOTest1 {
	...
	public void test() {
		...
		member.setUname("관리자");
		member.setId("admin");
		member.setEmail("admin@kgmail.com");
		member.setPhone("01012347777");
		member.setBirth("2022-07-12");
		DTOTest(member);
	}
	/*
	 ...
	 */
	public void DTOTest(MemberDTO member) {
		try {
			Assert.assertNotNull(member);	//값이 비어있는지 테스트
			Assert.assertEquals(member.getUname(), "관리자"); //특정 속성값의 일치 비교 테스트
		} catch(Exception e) {
